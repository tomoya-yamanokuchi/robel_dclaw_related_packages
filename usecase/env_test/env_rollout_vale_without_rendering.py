...
class EnvRolloutValve:
    ...
    def run(config):
        ...
        for n in range(episode):
            ...
            # env.render() # [use_render=False]なら使わない
            # ---
            for t in range(num_task_step):
                # render = env.render() # [use_render=False]なら使わない
                state  = env.get_state()
                print(f"object_position = {state['object_position'].value.squeeze()}")
                # env.view() # [use_render=False]なら使わない
                # ---
                env.set_task_space_ctrl(TaskSpacePosition(NTD(task_space_ctrl[t])))
                env.step()
            # ---
            render = env.render() # 使ってもエラーにはならないようにしているけど(NullRenderingManagerが後ろで動く) Noneが返ってくるだけ
            print(f"render = {render}") # --> None
...
if __name__ == '__main__':
    ...
    def main(config: DictConfig):
        '''
        レンダリングを使わない時のパラメータ設定
            1.          use_render = False
            2. viewer.is_Offscreen = True/False
        '''
