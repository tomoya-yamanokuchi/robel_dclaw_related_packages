    ...
    def run(config):
        ...
        for n in range(episode):
            ...
            for t in range(num_task_step):
                # render = env.render() # これをいれると env.render() が実行されるタイミングでmujoco viewerの画面がチカチカする...
                state  = env.get_state()
                env.view() # [use_render=True] & [is_Offscreen=False] ならmujocのviewer画面でレンダリングされる
                # ---
                env.set_task_space_ctrl(TaskSpacePosition(NTD(task_space_ctrl[t])))
                env.step(is_view=True) # Trueにしたら内部のimplicit step回す間の動きもレンダリングする
...
if __name__ == '__main__':
    ...
    def main(config: DictConfig):
        '''
        Mujoco Viewerでレンダリングするためのパラメータ設定
            1.          use_render = True
            2. viewer.is_Offscreen = False
        '''
